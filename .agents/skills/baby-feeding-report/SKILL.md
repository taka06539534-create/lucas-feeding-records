---
name: baby-feeding-report
description: Use when working in this repo on Chinese newborn feeding or baby care records, including daily file-based reports, baby-feeding-report tasks, README index updates, Markdown, visualizations, breastfeeding, bottle-fed breastmilk, formula, diapers, stool, urine, sleep, temperature, or abnormalities.
---

# 新生儿喂养记录日报

## 概览

将月嫂、家人或护理人员发来的中文半结构化记录整理成每日喂养护理报告。默认必须生成文件产物：新建报告文件夹、写入 `index.md`、结构化数据和可视化图表；聊天回复只提供简短完成说明和入口路径。医疗相关内容只做风险提醒，不能替代医生诊断。

本项目把 `$baby-feeding-report` 作为项目级入口。它继承全局 `baby-feeding-summarize` 的分析和渲染流程，并添加本仓库的宝宝信息、文件命名和索引同步规则。

## 项目默认要求

- 生成或更新报告前，先读取根目录 `baby-info.md`，并在报告中结合性别、出生日期、记录日龄和出生第几天。不要用系统当前日期代替记录日期。
- 原始解析输入保存为根目录 `<YYYY-MM-DD>-baby-feeding-report-input.json`；报告输出到 `reports/`，目录名包含记录日期和 `baby-feeding-report`。
- 默认运行本项目 skill 内脚本：`python .agents/skills/baby-feeding-report/scripts/render_visual_report.py --input <input-json> --output-dir reports --basename baby-feeding-report --screenshot auto`。
- 每次新增、更新、重生成或删除喂养记录、日报目录、结构化数据或可视化文件时，同步更新根目录 `README.md` 的“喂养记录索引”。
- 如果用户要求覆盖旧版本，删除或替换对应旧报告产物，不保留无效入口；如果同一天保留多个版本，在 `README.md` 中明确列出实际入口。
- 生成 SVG、HTML、PNG 或其他可视化视图时，长文本要主动换行或压缩成短句；生成后检查可视化文本长度，避免溢出容器。

## 何时使用

- 用户粘贴新生儿或婴儿每日喂养、护理、换尿不湿、睡眠、体温、异常等原始记录。
- 用户要求统计奶量、亲喂时长、喂养间隔、大小便次数、睡眠分布或护理要点。
- 用户要求生成日报、完整报告、总结、统计、图表、SVG、截图、可视化日报，或将可视化图表放入最终 Markdown 产物。
- 原文来自微信群、月嫂日报、育儿记录表，格式可能不完整或口语化。

## 何时不要使用

- 用户只是在咨询泛泛育儿知识，没有提供具体记录。
- 用户要求医疗诊断、用药建议、疾病处理方案。
- 用户要构建喂养记录 App、数据库或自动化系统，而不是分析一段护理记录。

## 核心流程

1. 先确认输入是否包含可分析的原始记录。没有记录时，请用户粘贴当天记录；多天记录按日期拆开分别总结。
2. 检查原文标题或用户说明中是否有记录日期。缺少日期时仍继续分析，但必须提醒用户补充日期；不要用当前日期、系统日期或聊天日期代替原始记录日期。
3. 标准化日期、时间、单位和同义词。把 `—`、空值、未写明、无法判断的字段标记为“未记录”或“未明确”，不要补造。
4. 按时间建立护理时间线。每个时间点可包含多类事件，例如喂奶、拍嗝、换尿不湿、睡眠、体温。
5. 分类统计喂养：
   - 亲喂母乳：统计次数、左/右侧时长、单次和总亲喂时长。
   - 瓶喂母乳：统计次数、每次 ml、总 ml。
   - 奶粉/配方奶：统计次数、每次 ml、总 ml。
   - 类型不清时标为“未明确”，不要自动归入奶粉或母乳。
6. 分析排便排尿：从“换尿不湿/换尿布/换纸尿裤”和“大便”两类记录合并判断次数、时间点、颜色、性状、量。凡是换尿布相关描述，未明确写“大便有/拉了/便便/臭臭”时，按小便 1 次计入；明确写大便有时计大便，是否同时计小便只看原文是否明确有尿/小便/尿布湿。
7. 推测睡眠：只根据记录计算。只有入睡没有醒来、只有睡姿没有时长、或缺少结束时间时，说明无法精确计算。
8. 始终进入文件产物模式，不要走“只在聊天中输出文本报告”的分支。先按 `references/parsing-and-output.md` 写出完整中文报告正文，再把这份正文放入结构化 JSON 的 `report_markdown` 字段。
9. 读取 `references/chart-spec.md`，整理结构化 JSON，并运行 `scripts/render_visual_report.py`。脚本必须在父级输出目录下新建包含记录日期的报告文件夹，把所有结果文件放入该文件夹，并以 `index.md` 作为默认访问入口。
10. 聊天回复只输出简短完成说明、`index.md` 路径和必要提醒；不要把完整报告正文直接贴在聊天中。

## 输出要求

`index.md` 使用中文 Markdown，默认包含以下小节，顺序不要改变。优先使用表格、列表和短句；连续正文段落不要超过 4 行。

1. `### 今日概览`
2. `### 喂养分析`
3. `### 排便排尿分析`
4. `### 睡眠推测`
5. `### 观察优先级`
6. `### 信息缺口`
7. `### 统计视图`

输出风格要求：

- 未识别到记录日期时，在 `### 今日概览` 前加一句短提醒：`> 提醒：这份记录未写明日期，请补充记录日期，便于判断是否跨日并做更准确的参考区间对比。`
- `今日概览` 用 3-5 条短句，每条只讲一个结论。
- `喂养分析` 必须包含当前状态判断，分清“较好/需要关注/可优化”，问题要结合具体记录证据展开说明，并给出可执行优化建议。
- `观察优先级` 用“继续保持 / 建议补充记录 / 需要观察 / 建议咨询医生”分级，不要把所有建议混在一起。
- 如果原始记录非常短，可以压缩表格行数，但不要省略不确定性说明。
- `信息缺口` 中优先列出缺失日期；不要擅自补写为今天、昨天或系统日期。
- `统计视图` 是最后一个章节，集中展示趋势速览、参考区间对比、关键数据和喂养时间线；其中参考区间对比必须标明“仅作常见参考，需结合日龄、体重、医嘱、喂养方式和宝宝状态”。

## 文件产物模式

任何可分析的喂养护理记录都默认进入文件产物模式，即使用户只说“总结”“分析”“完整报告”，也必须生成 `.md` 和可视化产物。

1. 先按输出要求生成完整中文报告正文，放入 JSON 的 `report_markdown` 字段；不要把完整正文作为聊天主回复。
2. 按 `references/chart-spec.md` 生成结构化 JSON。不要把未经确认的推测写成精确值。
3. 如果原文没有日期，JSON 的 `date` 写为 `日期未记录`，最终 Markdown 必须提醒用户补充记录日期。
4. 运行 `scripts/render_visual_report.py --input <json> --output-dir <parent-dir> --basename <name> --screenshot auto`。`--output-dir` 是父级输出目录；脚本会自动创建报告文件夹，不要把结果文件散放在父目录。
5. 报告文件夹命名必须包含记录日期，推荐 `<记录日期>-<basename>`；缺失日期时使用 `日期未记录-<basename>`；同名已存在时追加序号，避免覆盖旧报告。
6. 报告文件夹内至少包含 `index.md`、`structured-data.json`、`<name>.svg`、`<name>.html`，截图可用时还包含 `<name>.png`；默认把 `index.md` 作为访问入口。
7. `index.md` 先展示完整中文报告正文，最后只保留一个 `## 可视化图表` 章节，不要单独创建 `## 截图` 章节。
8. 如果脚本生成了 PNG 截图，在 `## 可视化图表` 中嵌入 PNG，并在同一小节提供 SVG 源文件链接。
9. 如果没有 PNG 截图，在 `## 可视化图表` 中直接嵌入 SVG，并说明缺少截图能力。
10. 如果脚本未生成 PNG，但浏览器/Chrome 插件可用，打开报告文件夹中的 `<name>.html`，截图保存为同文件夹的 `<name>.png`，然后重新运行脚本 `--screenshot never` 刷新 `index.md`。
11. 如果无法截图，不要阻塞任务；说明缺少浏览器截图能力，并保留 SVG、HTML、Markdown 产物。

不要把 Chrome、Browser、Playwright 等某个平台的工具假设为通用能力；只有当前环境明确可用时才使用。

## 安全边界

- 不要把亲喂母乳时长强行换算为 ml，除非用户明确提供换算规则。
- 不要把推测睡眠写成精确监测结果。
- 不要根据单日记录下诊断结论。
- 不要把常见参考区间写成硬性标准；不足或超出参考区间只能提示“建议结合宝宝状态继续观察/咨询医生”。
- 若记录出现持续发热、精神差、明显喂养困难、尿量明显减少、血便、黑便、白陶土样便、频繁喷射性呕吐、呼吸异常、反复呛奶等信号，应建议及时咨询儿科医生或就医。
- 建议必须基于记录本身，用“建议观察”“可与医生确认”“后续记录可补充”这类谨慎表达。
- 涉及睡姿时，若记录出现侧卧或趴睡，温和提示权威安全睡眠建议通常强调仰卧入睡；不要责备照护者。

## 常见坑

- “奶粉”“配方奶”可以合并为奶粉/配方奶；“瓶喂母乳”“吸出母乳”不能和奶粉合并。
- 换尿布相关描述只要未明确写“大便有/拉了/便便/臭臭”，就按小便 1 次计入；例如“换尿不湿”“换尿布：大便无”“换纸尿裤，未见大便”都计小便。
- “换尿不湿：大便有”计大便；是否同时计小便只看原文是否明确写“小便有/有尿/尿布湿”等线索。
- “大便：颜色黄，性状糊，量中”是大便质量记录；如果同一时间已有“换尿不湿：大便有”，不要重复计为两次，除非原文明确是两次。
- “异常：无”表示未记录异常；“异常：—”表示异常字段未填写。
- 夜间跨日时要说明日期边界；无法确认是否属于同一天时，保留原时间并标注不确定。
- “喝母乳 70 ml”通常表示瓶喂母乳；“吸母乳左右各 15 分钟”表示亲喂母乳。
- “豪升”多半是“毫升”的误写，可标准化为 ml，并在信息缺口中建议统一单位。

## 验证

创建或更新本 skill 后，至少用 `evals/` 中的用例检查：

- 普通完整记录能输出所有指定小节。
- 普通完整记录默认生成报告文件夹和 `index.md`，不得只在聊天中输出文本报告。
- 缺失、占位符和歧义记录不会被编造。
- 输出先给状态判断和优化建议，最后用统计视图集中展示趋势表、参考区间对比、关键数据和时间线。
- 文件产物模式能生成 SVG，并在截图能力可用时生成 PNG，最终 Markdown 能嵌入图表。
- 每次运行都会新建包含记录日期的报告文件夹，并以 `index.md` 作为默认访问入口。
- 非记录类育儿或产品需求不会误触发本 skill。
